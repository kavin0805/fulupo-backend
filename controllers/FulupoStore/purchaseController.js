  ...
  try {
    ...
    const enrichedProducts = await Promise.all(
      parsedProducts.map(async (p) => {
        ...
        console.log(
          "LED SPECIAL PEABERRY COFFEE POWDER (KG)",
          storeProduct,
          p.product_name
        );
        ...
        if (!storeProduct) {
          // masterProduct = await MasterProducts.findOne({
          //   name: { $regex: new RegExp(`^${p.product_name}$`, 'i') }
          // });
          ...
          if (!masterProduct) {
            ...
            masterProduct = new MasterProducts({
              name: p.product_name,
              productCode,
              categoryId: p.categoryId || "68888fd8aae4b2e01c991d8e", // fallback category
              productImage: p.product_img || "",
        ...
        if (inventory) {
          ...
          inventory.product_amount = p.product_amount;
            await inventory.save();
        } else {
          inventory = new Inventory({
            storeId,
