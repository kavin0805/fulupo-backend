...
export const getWastage = async (req, res) => {
  try {
    ...
    const data = await Wastage.find(filter)
      .populate('storeId', 'store_name')
      .populate('productList.product_id', 'name productImage' );
...
export const getWastageByProduct = async (req, res) => {
  try {
    ...
    const data = await Wastage.find({ storeId , 'productList.product_id': product_id })
      .populate('storeId', 'store_name')
      .populate('productList.product_id', 'name productImage');
    res.json({ data });
  } catch (err) {
