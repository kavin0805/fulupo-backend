...
export const addVendor = async (req, res) => {
  try {
    const { storeId, vendorName, vendorGst, vendorMobile, vendorLandline, vendorAddress } = req.body;
    ...
    res.status(500).json({ message: 'Error adding vendor', error: err.message });
  }
};
...
export const getPurchasesByVendorName = async (req, res) => {
  try {
    ...
    if (!storeId || !vendorName)
      return res.status(400).json({ message: 'storeId and vendorName are required' });

    const data = await Purchase.find({ storeId, vendorName })
      .populate('product.product_id', 'name productImage productCode');

    res.json({ count: data.length, data });
  } catch (err) {
    res.status(500).json({ message: 'Error fetching vendor purchases', error: err.message });
  }
};
...
export const getVendorsByProductId = async (req, res) => {
  try {
    ...
    if (!storeId || !productId)
      return res.status(400).json({ message: 'storeId and productId are required' });
    ...
    res.status(500).json({ message: 'Error fetching vendors by product', error: err.message });
  }
};
